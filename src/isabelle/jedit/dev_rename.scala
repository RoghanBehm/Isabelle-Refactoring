...
object Refactor {
  ...
  def find_occurrences(
    ...
    // Match known derived names (e.g. myConst_def, myConst_simps)
    val matches_derived: String => Boolean = name =>
      target_name.exists { t =>
        derived_suffixes.exists { suffix =>
          name == t + suffix ||
          name.endsWith("." + t + suffix)
        }
      }
...
º    for {
  ...
  /** Walk upward from `start` looking for a session root directory.
    * Prefers directories containing ROOTS; falls back to ROOT.
    */
